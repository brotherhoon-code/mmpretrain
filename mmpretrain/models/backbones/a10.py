...
from mmpretrain.models.backbones.custom_modules.attention import CBAM_C
...
class ChannelMixBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.channel_mix_layer = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1))
        self.active_func = nn.GELU()
        self.bn_layer = nn.BatchNorm2d(num_features=out_channels)
        self.attn = CBAM_C(in_channels = out_channels, fusion="add")
...
class ProjBlock(nn.Module):
    def __init__(self, in_channels:int, out_channels:int):
        super().__init__()
        self.proj = nn.Conv2d(in_channels, out_channels, kernel_size=(1, 1), bias=False)
...
class MixerBlock(nn.Module):
    def __init__(self, dim: int, kernel_size: int):
        ...
        self.spatial_mix = SpatialMixBlock(dim = dim, kernel_size = kernel_size)
        self.channel_mix = ChannelMixBlock(in_channels = dim, out_channels = dim)

    def forward(self, x):
        x = self.spatial_mix(x) # delete skip conn
