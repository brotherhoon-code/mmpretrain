...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_attn_map = self.channel_attn_block(x)
        if channel_attn_map.dim() == 1:
            channel_attn_map = repeat(channel_attn_map, 'b -> b c h w', c=x.size(1), h=x.size(2), w=x.size(3))
        else:
            channel_attn_map=repeat(channel_attn_map, 'b c -> b c h w', h=x.size(2), w=x.size(3))
...
if __name__ == "__main__":
    ...
    m = CustomCBAM(BottleNeckBlock, [3,4,6,3], n_classes=100)
    summary(m, (3,32,32), device='cpu', batch_size=1)
