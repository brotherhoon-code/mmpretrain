...
# SEBlock은 b c h w 피처맵을 받아 b c 차원의 output을 내보냅니다
class TanhReLU(nn.Module):
    ...
    def forward(self, x):
        x = self.bn(x)
        x = self.temperature * F.tanh(x/self.temperature)
        x = F.relu(x)
        return x
...
class SEblock(nn.Module):
    def __init__(self, 
        ...
        if activation_func == 'sigmoid':
            ...
            self.activation_func = TanhReLU(in_channels)
        else:
            raise ValueError(f"not support activation func {activation_func}")
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, 
        ...
        self.seblock = SEblock(channels*self.expansion,r=16, activation_func=activation_func)
        ...
        if fusion_type == 'scale':
            self.fusion_type = 'scale'
        elif fusion_type == 'add':
            self.fusion_type = 'add'
        else:
            raise ValueError(f"not supprot fusion type {fusion_type}")
...
class CustomSEResNet(nn.Module):
    def __init__(self, 
                 block_cls:Type[Union[BasicBlock,BottleNeckBlock]] = BottleNeckBlock, 
        ...
        self.stage1_block = self._make_stage(self.block_cls, 64, 64, n_blocks[0], 1, activation_func, fusion_type)
        self.stage2_block = self._make_stage(self.block_cls, 64*self.expansion, 128, n_blocks[1], 2, activation_func, fusion_type)
        self.stage3_block = self._make_stage(self.block_cls, 128*self.expansion, 256, n_blocks[2], 2, activation_func, fusion_type)
        self.stage4_block = self._make_stage(self.block_cls, 256*self.expansion, 512, n_blocks[3], 2, activation_func, fusion_type)
    ...
    @staticmethod
    def _make_stage(block:BottleNeckBlock, in_channels, out_channels, num_blocks, stride, activation_func = 'sigmoid', fusion_type = 'scale'):
