...
class SpatialSelfConv(nn.Module):
    def __init__(
        ...
        self.mlp0 = nn.Linear(height * width, kernel_size * kernel_size)
        
        self.reshape1 = Rearrange("B C N -> B N C")
        self.mlp1 = nn.Linear(channel, int(channel//2)) # bottleneck
        self.dropout = nn.Dropout(p=dropout_ratio)
        self.activ_func = get_activ_func(activ_func)
        self.mlp2 = nn.Linear(int(channel//2), channel) # bottleneck
        
        self.reshape2 = Rearrange(
            "B (K_H K_W) C -> (B C) 1 K_H K_W", K_H=kernel_size, K_W=kernel_size
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        ## (start) make kernel
        attn = self.reshape0(x) # B C N
        attn = self.layernorm(attn) # B C N
        attn = self.mlp0(attn)# B C KK  
        attn = self.reshape1(attn) # B KK C
        attn = self.mlp1(attn) # B KK c
        attn = self.dropout(attn) # B KK c
        attn = self.activ_func(attn)  # B KK c
        attn = self.mlp2(attn)  # B KK C
        attn_weight = self.reshape2(attn) # B C K K
        ## (end) make kernel
