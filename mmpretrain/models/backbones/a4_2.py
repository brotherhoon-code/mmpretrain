...
from mmpretrain.models.backbones.custom_modules.norm import LayerNorm
...
class PatchMergeBlock(nn.Module):
    def __init__(self, in_channels:int, out_channels:int, patch_size:int):
        ...
        self.norm = LayerNorm(normalized_shape = in_channels)
...
class A4_2(nn.Module):
    def __init__(self,
        ...
        self.s2_patch_merge = PatchMergeBlock(in_channels=stage_channels[0], out_channels=stage_channels[1], patch_size=patch_size[1])
        self.stage2 = nn.Sequential(*[MixerBlock(stage_channels[1], kernel_size) for _ in range(stage_blocks[1])])
        
        self.s3_patch_merge = PatchMergeBlock(in_channels=stage_channels[1], out_channels=stage_channels[2], patch_size=patch_size[2])
        self.stage3 = nn.Sequential(*[MixerBlock(stage_channels[2], kernel_size) for _ in range(stage_blocks[2])])
        
        self.s4_patch_merge = PatchMergeBlock(in_channels=stage_channels[2], out_channels=stage_channels[3], patch_size=patch_size[3])
    ...
    def forward(self, x):
        ...
        x = self.s2_patch_merge(x)
        x = self.stage2(x)
        outs.append(x)
        
        x = self.s3_patch_merge(x)
        x = self.stage3(x)
        outs.append(x)
        
        x = self.s4_patch_merge(x)
        x = self.stage4(x)
        outs.append(x)
...
if __name__ == "__main__":
    ...
    summary(m, (3, 224, 224), batch_size=256, device="cpu") 
