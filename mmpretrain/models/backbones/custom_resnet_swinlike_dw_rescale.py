...
from mmpretrain.models.backbones.custom_modules.attention import SEModule, CBAM_C, CBAM_S
...
class PatchEmbed(nn.Module):
    def __init__(self,
                 ...
                 act_func:str="ReLU"):
        super().__init__()
        self.embed_layer = nn.Conv2d(in_channels, out_channels, kernel_size=patch_size, stride=patch_size)
...
class BottleneckResBlock(nn.Module):
    def __init__(self, 
        ...
        attn_type = kwargs.get('attn_type', 'SE') # keyword, default
        ...
        self.attn_block = SEModule(in_channels=out_channels,
                                   r=16,
                                   activ_func="ReLU") if attn_type=="SE" else nn.Sequential(CBAM_C(in_channels=out_channels),
                                                                                            CBAM_S(kernel_size=7))
...
class SwinLikeRescaleDWResNet(nn.Module):
    def __init__(self, 
        ...
        # 96 -> 192
        self.patch_embed2 = PatchEmbed(stage_out_channels[0], stage_out_channels[0]*2, 2, act_func)
        # 192 -> 384
        self.patch_embed3 = PatchEmbed(stage_out_channels[1], stage_out_channels[1]*2, 2, act_func)
        # 384 -> 768
        self.patch_embed4 = PatchEmbed(stage_out_channels[2], stage_out_channels[2]*2, 2, act_func)
        
        self.stage1 = nn.Sequential(
            *[block(in_channels = stem_channels,
                    inter_channels = feature_channels[0],
                    out_channels = stage_out_channels[0],
                    stride = strides[0], 
                    act_func = act_func,
                    dw = dw[i],
                    attn_type=attn_type) for i in range(stage_blocks[0])]
            )
        self.stage2 = nn.Sequential(
            *[block(in_channels = stage_out_channels[0]*2,
                    inter_channels = feature_channels[1],
                    out_channels = stage_out_channels[1],
                    stride = strides[1],
                    act_func = act_func,
                    dw = dw[i],
                    attn_type=attn_type) for i in range(stage_blocks[1])]
            ) 
        self.stage3 = nn.Sequential(
            *[block(in_channels = stage_out_channels[1]*2,
                    inter_channels = feature_channels[2],
                    out_channels = stage_out_channels[2],
                    stride = strides[2],
                    act_func = act_func,
                    dw = dw[i],
                    attn_type=attn_type) for i in range(stage_blocks[2])]
            ) 
        self.stage4 = nn.Sequential(
            *[block(in_channels = stage_out_channels[2]*2,
                    inter_channels = feature_channels[3],
                    out_channels = stage_out_channels[3],
                    stride = strides[3],
                    act_func = act_func,
                    dw = dw[i],
                    attn_type=attn_type) for i in range(stage_blocks[3])]
            ) 
...
if __name__ == "__main__":
    ...
    m = SwinLikeRescaleDWResNet(dw = [True, True, True], attn_type = "CBAM")
    ...
    # SE
    # Trainable params: 6,633,486
    # CBAM
    # Trainable params: 6,933,024
