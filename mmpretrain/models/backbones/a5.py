...
class MixerBlock(nn.Module):
    def __init__(self, dim: int, kernel_size: int):
        ...
        self.proj = ProjBlock(in_channels=dim, out_channels=dim//2)
        self.spatial_mix = SpatialMixBlock(dim = dim//2, kernel_size = kernel_size)
        self.channel_mix = ChannelMixBlock(in_channels = dim//2, out_channels = dim)

    def forward(self, x):
        x = self.proj(x)
        x = self.spatial_mix(x) # + x # delete skip conn
