...
class ConcatNonLocalBlock(nn.Module):
    def __init__(self, 
       ...
       self.concat_project_layer = nn.Sequential(nn.Conv2d(in_channels=embed_dim*2, 
                                                           ...
                                                           kernel_size=(1,1), 
                                                           bias=False), 
                                                 nn.ReLU(inplace=True)) # B 2c H W -> B 1 H W
       # W
       self.expansion_layer = nn.Conv2d(in_channels=embed_dim, 
                                         out_channels=in_channels, 
                                         kernel_size=(1,1)) # B C H W
...
class CustomNonLocalResNet(nn.Module):
    def __init__(self, 
                 ...
                 **kwargs):
        super(CustomNonLocalResNet, self).__init__()
...
#     m = CustomNonLocalResNet(block_cls=BottleNeckBlock, 
#                              n_blocks=[3,4,6,3], 
#                              n_classes=10, 
#                              n_local_cls="ConcatNonLocalBlock",
#                              n_local_stage_idx=[1,2,3,4]).cuda()    
#     summary(m, (3,32,32), batch_size=128, device="cuda")
    
    # EmbeddedGaussianNonLocalBlock: 34,705,802
    # GaussianNonLocalBlock: 29,131,402
    # DotProductNonLocalBlock: 34,705,802
    # ConcatNonLocalBlock: 34,709,642
