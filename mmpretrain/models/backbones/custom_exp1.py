...
class BottleneckResBlock(nn.Module):
    def __init__(self, 
        ...
        self.block = nn.Sequential(
            ...
            nn.Conv2d(in_channels=inter_channels, out_channels=inter_channels, kernel_size=3, groups=inter_channels, padding=1),
            nn.BatchNorm2d(inter_channels),
            getActFunc(act_func),
...
class DWResNet(nn.Module):
    def __init__(self, 
        ...
        self.stage1 = nn.Sequential(
            *[block(stem_channels, feature_channels[0], stage_out_channels[0], 1, act_func) for i in range(stage_blocks[0])]
            )
        self.stage2 = nn.Sequential(
            *[block(stage_out_channels[0]*2, feature_channels[1], stage_out_channels[1], 1, act_func) for i in range(stage_blocks[1])]
            ) 
        self.stage3 = nn.Sequential(
            *[block(stage_out_channels[1]*2, feature_channels[2], stage_out_channels[2], 1, act_func) for i in range(stage_blocks[2])]
            ) 
        self.stage4 = nn.Sequential(
            *[block(stage_out_channels[2]*2, feature_channels[3], stage_out_channels[3], 1, act_func) for i in range(stage_blocks[3])]
            ) 
