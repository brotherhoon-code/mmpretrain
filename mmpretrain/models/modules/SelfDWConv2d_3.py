...
class SelfDWConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.linear0 = nn.Linear(
            in_features=kernel_size**2, out_features=kernel_size, bias=False
        )
        self.norm1 = nn.LayerNorm([in_channels, kernel_size])
        self.activ_func = nn.Sigmoid()
        self.linear1 = nn.Linear(
            in_features=kernel_size, out_features=kernel_size**2, bias=False
        )
        self.reshape1 = Rearrange(
            "b c (kh kw) -> b c kh kw", kh=kernel_size, kw=kernel_size
        )
    ...
    def forward(self, x):
        ...
        x = self.activ_func(x)
        x = self.linear1(x)
        bias = self.weight.unsqueeze(0).expand(B, -1, -1, -1)
        x = self.reshape1(x) + bias
