...
class SelfConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, temp=20.0, pooling_resolution=7):
        ...
        self.q_layer = nn.Linear(in_features=pooling_resolution**2, out_features=pooling_resolution**2, bias=False)
        self.q_reshaper = Rearrange("b c p_h p_w -> b c (p_h p_w)")  # b c 49

        self.k_reshaper1 = Rearrange("b c p_h p_w -> b c (p_h p_w)")  # b n c
        self.k_layer = nn.Linear(in_features=pooling_resolution**2, out_features=pooling_resolution**2, bias=False)
        self.k_reshaper2 = Rearrange("b c n -> b n c")  # b n c
    ...
    def _get_query(self, pooled_feature_map: torch.Tensor):
        Q = self.q_reshaper(pooled_feature_map) # b c n
        Q = self.q_layer(Q) # b c n
        return Q

    def _get_key(self, pooled_feature_map: torch.Tensor):
        K = self.k_reshaper1(pooled_feature_map) # b c n
        K = self.k_layer(K) # b c n
        K = self.k_reshaper2(K) # b n c
        return K
    ...
    def forward(self, x: torch.Tensor):
        ...
        kernel_weights = self.fusion(kernel_weights)
