...
class SelfConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, temp=7.0, pooling_resolution=7, bottle_ratio=4):
        ...
        self.kernel_weights = nn.Parameter(torch.Tensor(in_channels, kernel_size, kernel_size)) # c, k, k
        self.kernel_reshaper = Rearrange('b c p_h p_w -> b c (p_h p_w)') # b c 49
        nn.init.kaiming_uniform_(self.kernel_weights, mode='fan_in', nonlinearity='relu') # init
    ...
    def _get_kernel(self, num_batch):
        kernel_weights = self.kernel_weights
        kernel_weights = repeat(kernel_weights, 'c k_h k_w -> b c k_h k_w', b=num_batch)
    ...
    def forward(self, x: torch.Tensor):
        ...
        kernel_weights = self.filter_reshaper(kernel_weights) # b c k k
...
if __name__ == "__main__":
    ...
    B, C, H, W = input.shape
    self_conv = SelfConv2d(in_channels=C, out_channels=C, kernel_size=7)
