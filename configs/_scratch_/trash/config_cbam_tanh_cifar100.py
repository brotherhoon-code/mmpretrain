            ...
            hooks=[
                dict(type='MMClsWandbHook',
                     init_kwargs={
                         ...
                         'name': 'cbam50_tanh', # wandb 기록될 이름
                         'notes': 'act func: channel(tanh) + spatial(sigmoid)'}, # 메모
                     log_checkpoint=True,
...
resume_from = None
workflow = [('train', 1)]
work_dir = './work_dir/cbam_tanh_sigmoid_cifar100' # config_를 제외한 파일명
gpu_ids = [0]
visualizer = dict(
    type='ClsVisualizer',
    vis_backends=[
